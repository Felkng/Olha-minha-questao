...
export const EditFlashcardModal: React.FC<EditFlashcardModalProps> = ({
  ...
  useEffect(() => {
    if (open && flashcard) {
      ...
      setAreaId(flashcard.areaId || '');
      setSubjectId(flashcard.subjectId || '');
      setFolderId(flashcard.folderId || '');
      setIsPublic(flashcard.isPublic ?? true);
      loadOptions(flashcard.areaId);
    }
  }, [open, flashcard, user?.id]);

  const loadOptions = async (initialAreaId?: number) => {
    try {
      ...
      if (initialAreaId) {
        const sList = await getSubjectsByArea(initialAreaId);
        setSubjects(sList);
      }
    } catch (err) {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle sx={{ fontWeight: 800 }}>Editar Flashcard</DialogTitle>
